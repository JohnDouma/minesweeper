...
public class Board {
    ...
    // Abstraction Function: A 2D array of squares represents a Minesweeper board
    ...
    // Representation Exposure Argument: The array of squares is not accessible to clients. 
    // The state of the board is only viewed through its toString method.
    
    // Thread Safety Argument: All public non-constructor methods are synchronized
    
    private enum State {UNTOUCHED, DUG, FLAGGED};
    ...
    public Board(int rows, int cols) {
        ...
        // TODO checkRep() should adjust the bombsWithNeighbors values
    }
    
    /**
     * Attempts to dig at a board location. If the square specified
     * by x and y is valid and in an undug state, sets the state of the
     * square to DUG and removes the bomb if it contains one.
     * 
    ...
    /*
     * Returns true if x and y specify a valid location on the grid.
    ...
    /*
     * If the specified grid location contains no neighbors with bombs then
     * change the state of all untouched neighbors to dug and recursively perform
     * this step for each of their neighbors.
     */
    ...
    private Queue<Pair> generateNeighbors(int x, int y) {
        ...
        if (x > 0) {
            ...
            if (y > 0) queue.add(new Pair(x-1, y-1));
            if (y < grid[0].length-1) queue.add(new Pair(x-1, y+1));
        }
